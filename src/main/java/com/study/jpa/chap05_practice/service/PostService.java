...
import com.study.jpa.chap05_practice.repository.HasTagRepository;
...
@Transactional  // JPA repository는 트랜잭션 단위로 동작하기 때문에
public class PostService {
    ...
    public PostListResponseDTO getPosts(PageDTO dto) {
        ...
        return PostListResponseDTO.builder()
                ...
                // JPA가 준 페이지 정보가 담긴 객체를 DTO에 전달해서 알고리즘 돌리기
    ...
    public PostDetailResponseDTO insert(PostRequestDTO dto) throws Exception {
        // 게시물 저장 (해시태그는 저장되지 않음)
        Post saved = postRepository.save(dto.toEntity());

        // 해시태그 저장
        List<String> hasTags = dto.getHasTags();
        if (hasTags != null && !hasTags.isEmpty()) {
            hasTags.forEach(ht -> {
                HashTag hasTag = HashTag.builder()
                        .tagName(ht)
                        .post(saved)
                        .build();
                HashTag savedTag = hasTagRepository.save(hasTag);

                // Post에도 해시태그 추가
                saved.addHasTag(savedTag);
            });
        }
        ...
        /*
            ...
            (Post를 다시 SELECT해서 가져온다? -> INSERT가 완료된 후에 SELECT -> Entity Manager로 강제 flush()
             INSERT는 트랜잭션 종료 후 실행.)
        */

        // insert 요청한 게시물 정보를 DTO로 변환해서 리턴
